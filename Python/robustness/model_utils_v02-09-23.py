    ...
    if resume_path and os.path.isfile(resume_path):
        ...
        if not ('state_dict' in checkpoint):
            ...
            if DEFENSE == 'Locus':
                ...
                checkpoint_attacker = {'attacker.'+k:v for k,v in checkpoint.items()}
                checkpoint = dict(checkpoint, **checkpoint_attacker)
                if DATASET == 'cifar10':
                ...
                checkpoint = dict(checkpoint, **extra)
            if DEFENSE == 'Hadi':
                # potentiell Namensanpassungen
                if DATASET == 'cifar10':
                    extra = {'normalizer.new_mean': ch.Tensor([[[0.4914]], [[0.4822]], [[0.4465]]]),
                             'normalizer.new_std': ch.Tensor([[[0.247]], [[0.243]], [[0.261]]]),
                             'attacker.normalize.new_mean': ch.Tensor([[[0.4914]], [[0.4822]], [[0.4465]]]),
                    ...
                    extra = {'normalizer.new_mean': ch.Tensor([[[0.485]], [[0.456]], [[0.406]]]),
                             'normalizer.new_std': ch.Tensor([[[0.229]], [[0.224]], [[0.225]]]),
                             'attacker.normalize.new_mean': cch.Tensor([[[0.485]], [[0.456]], [[0.406]]]),
                             'attacker.normalize.new_std': ch.Tensor([[[0.229]], [[0.224]], [[0.225]]])}
                checkpoint = dict(checkpoint, **extra)
            sd = checkpoint
            # print("Checkpoint", checkpoint)
            ...
            if DEFENSE == 'Locus':
                # state_dict_path gibt Gewichte des Modells
                # bei Locus nur die Gewichte im dict, keine Epoche o.Ä.
                sd = checkpoint[state_dict_path]
                sd = {'model.' + k: v for k, v in checkpoint.items()}
                sd_attacker = {'attacker.' + k: v for k, v in checkpoint.items()}
                sd = dict(sd, **sd_attacker)
                if DATASET == 'cifar10':
                    extra = {'normalizer.new_mean': ch.Tensor([[[0.4914]], [[0.4822]], [[0.4465]]]),
                             'normalizer.new_std': ch.Tensor([[[0.247]], [[0.243]], [[0.261]]]),
                             'attacker.normalize.new_mean': ch.Tensor([[[0.4914]], [[0.4822]], [[0.4465]]]),
                    ...
                    extra = {'normalizer.new_mean': ch.Tensor([[[0.485]], [[0.456]], [[0.406]]]),
                             ...
                             'attacker.normalize.new_std': ch.Tensor([[[0.229]], [[0.224]], [[0.225]]])}
                sd = dict(sd, **extra)
        #### Vorsicht viel Handcrafted
        if DEFENSE == 'Madry':
            sd = {k[len('module.'):]:v for k,v in sd.items()} # removes 'module.' from everything in dictionary
            sd = {'model.' + k: v for k, v in checkpoint.items()}
            sd_attacker = {'attacker.' + k: v for k, v in checkpoint.items()}
