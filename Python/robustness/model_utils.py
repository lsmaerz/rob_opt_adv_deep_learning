...
def make_and_restore_model(*_, arch, dataset, resume_path=None,
    ...
    if resume_path and os.path.isfile(resume_path):
        ...
        state_dict_path = 'model' # location of model parameters within the checkpoint dictionary
        ...
        if DEFENSE == 'Hadi':
            if DATASET == 'cifar10':
                ...
                for k in checkpoint[state_dict_path]:
                    k_without_prefix = k[len('1.'):]
                    v = checkpoint[state_dict_path][k]
                    if k.startswith('1.'):
                        sd['model.' + k_without_prefix] = v
                        sd['attacker.model.' + k_without_prefix] = v
                    else:
                        sd[k] = v
                # extra = {'normalizer.new_mean': ch.Tensor([[[0.4914]], [[0.4822]], [[0.4465]]]),
                #          'normalizer.new_std': ch.Tensor([[[0.247]], [[0.243]], [[0.261]]]),
                #          'attacker.normalize.new_mean': ch.Tensor([[[0.4914]], [[0.4822]], [[0.4465]]]),
                #          'attacker.normalize.new_std': ch.Tensor([[[0.247]], [[0.243]], [[0.261]]])}
                # Hadi already has normalization layer!
                extra = {'normalizer.new_mean': ch.Tensor([[[0]], [[0]], [[0]]]),
                         'normalizer.new_std': ch.Tensor([[[1]], [[1]], [[1]]]),
                         'attacker.normalize.new_mean': ch.Tensor([[[0]], [[0]], [[0]]]),
                         'attacker.normalize.new_std': ch.Tensor([[[1]], [[1]], [[1]]])}
                ...
                for k in checkpoint[state_dict_path]:
                    ...
                    if k.startswith('1.module.'):
                        ...
                        sd[k] = v
                # extra = {'normalizer.new_mean': ch.Tensor([[[0.485]], [[0.456]], [[0.406]]]),
                #          'normalizer.new_std': ch.Tensor([[[0.229]], [[0.224]], [[0.225]]]),
                #          'attacker.normalize.new_mean': ch.Tensor([[[0.485]], [[0.456]], [[0.406]]]),
                #          'attacker.normalize.new_std': ch.Tensor([[[0.229]], [[0.224]], [[0.225]]])}
                # Hadi already has normalization layer!
                extra = {'normalizer.new_mean': ch.Tensor([[[0]], [[0]], [[0]]]),
                         'normalizer.new_std': ch.Tensor([[[1]], [[1]], [[1]]]),
                         'attacker.normalize.new_mean': ch.Tensor([[[0]], [[0]], [[0]]]),
            ...
            if DATASET == 'ImageNet':
                ...
                for k in checkpoint[state_dict_path]:
                    ...
                    if k.startswith('module.'):
                        sd['model.' + k_without_module] = v
                        sd['attacker.model.' + k_without_module] = v
                    else:
                        sd[k] = v
                extra = {'normalizer.new_mean': ch.Tensor([[[0.485]], [[0.456]], [[0.406]]]),
                         'normalizer.new_std': ch.Tensor([[[0.229]], [[0.224]], [[0.225]]]),
        ...
        if DEFENSE in  ['Natural', 'Madry']:
            ...
            for k in checkpoint[state_dict_path]:
                ...
                if k.startswith('module.'):
                    ...
                    sd[k] = v
            # # sd = {k[len('module.'):]: v for k, v in checkpoint.items()}  # removes 'module.' from everything in dictionary
            # sd = {k[len('module.'):]: v for k, v in checkpoint.items() if k.startswith('module.')}
            # sd.update({k: v for k, v in checkpoint.items() if not k.startswith('module.')})
            # sd = {'model.' + k: v for k, v in sd.items()} # prepends 'model.'
            # sd_attacker = {'attacker.' + k: v for k, v in sd.items()} # prepends 'attacker.'
            # sd_total = dict(sd, **sd_attacker) # unify both dictionaries
            # checkpoint[state_dict_path] = sd
